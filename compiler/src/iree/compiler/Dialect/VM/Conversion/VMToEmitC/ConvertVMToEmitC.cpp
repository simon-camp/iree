...
createModuleStructure(IREE::VM::ModuleOp moduleOp,
  ...
  SmallVector<Operation *> opsToRemove;
  {
    ...
    mark(op);
    op = builder.create<emitc::VerbatimOp>(loc, "extern \"C\" {", false);
    mark(op);
    op = emitc_builders::preprocessorDirective(
    ...
    for (auto rodataOp : moduleOp.getOps<IREE::VM::RodataOp>()) {
      ...
      stmt += "}";
      auto op = builder.create<emitc::VerbatimOp>(loc, stmt, true);
      mark(op);
    ...
    deps += "}";
    op = builder.create<emitc::VerbatimOp>(loc, deps, true);
    mark(op);
    ...
    imports += "}";
    op = builder.create<emitc::VerbatimOp>(loc, imports, true);
    mark(op);
    ...
    exports += "}";
    op = builder.create<emitc::VerbatimOp>(loc, exports, true);
    mark(op);
    ...
    functions += "}";
    op = builder.create<emitc::VerbatimOp>(loc, functions, true);
    mark(op);
    ...
    op = builder.create<emitc::VerbatimOp>(loc, descriptor, true);
    mark(op);
