...
LogicalResult convertFuncOp(IREE::VM::FuncOp funcOp,
  ...
  for (int i = 0; i < numLocalRefs; i++) {
    ...
    if (failed(IREE::VM::clearStruct(builder, refPtr))) {
      return failure();
    }
  }
...
LogicalResult retainOrMoveRefs(OpBuilder &builder, Location location,
  ...
  for (auto &[srcRef, destRef] : mapping.getValueMap()) {
    ...
    if (failed(IREE::VM::clearStruct(builder, tmpPtr))) {
      return failure();
    }
...
public:
  ...
  void runOnOperation() override {
    ...
    // Generate boilerplate code the iree API. Examples of this are include
    // guards, header includes, struct definitions and global variables.
