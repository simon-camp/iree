...
iree_status_t Run() {
  ...
  // Create module and append to the session.
  iree_vm_module_t* c_module = NULL;
  if (iree_status_is_ok(status)) {
    status = module_create(iree_allocator_system(), &c_module);
  }
  if (iree_status_is_ok(status)) {
    status = iree_runtime_session_append_module(session, c_module);
  }
