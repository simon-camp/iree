...
static bool isStructurallyEquivalentTo(Region &lhs, Region &rhs,
  ...
  BlockAndValueMapping scopedMapping;
  BlockAndValueMapping mapping =
      lhs.isIsolatedFromAbove() ? scopedMapping : parentMapping;

  // Regions can have arguments; ensure they match in type and add the mapping
  // from lhs -> rhs so that we can track the usage of them within our
  // descendants.
  if (lhs.getNumArguments() != rhs.getNumArguments()) return false;
  for (auto argPair : llvm::zip(lhs.getArguments(), rhs.getArguments())) {
  ...
  // Walk each block list together. If the sizes differ we'll handle that after
  // Build the block mapping so that when we recursive into ops we are able to
  // map lhs->rhs blocks regardless of their order in the region.
  //
  // We walk each block list simultaneously and handle length differences after
  // the loop; this avoids the need for doing an O(n) linked-list size query.
  if (!compare_ranges(
...
static bool isStructurallyEquivalentTo(Operation &lhs, Operation &rhs,
  ...
  // If the operation is isolated we don't want to reuse any parent mapping or
  // pollute it with our mappings.
  BlockAndValueMapping scopedMapping;
  BlockAndValueMapping &mapping =
      lhs.isKnownIsolatedFromAbove() ? scopedMapping : parentMapping;
  ...
  for (auto operandPair : llvm::zip(lhs.getOperands(), rhs.getOperands())) {
    ...
    if (lhsValue.getType() != rhsValue.getType()) return false;
    if (rhsValue != mapping.lookup(lhsValue)) return false;
  }
  ...
  for (auto regionPair : llvm::zip(lhs.getRegions(), rhs.getRegions())) {
    ...
    if (!isStructurallyEquivalentTo(lhsRegion, rhsRegion, mapping)) {
      return false;
    }
  }
