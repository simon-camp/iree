...
iree_status_t _TfLiteTensorReallocateIfNeeded(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_allocator_compute_size(buffer_allocator, shape_dims,
                                          tensor->shape_rank, element_type,
                                          &allocation_size));
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_buffer_map(tensor->buffer, IREE_HAL_MEMORY_ACCESS_ALL, 0,
...
void _TfLiteTensorDiscardBuffer(TfLiteTensor* tensor) {
  ...
  if (tensor->buffer_mapping.contents.data != NULL) {
    IREE_IGNORE_ERROR(
        iree_hal_buffer_unmap(tensor->buffer, &tensor->buffer_mapping));
  }
