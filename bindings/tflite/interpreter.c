...
// TODO(#3977): real registration and driver selection.
...
static iree_status_t _TfLiteInterpreterPrepareHAL(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_vmla_driver_module_register());
  IREE_RETURN_IF_ERROR(
      iree_hal_driver_registry_create_driver(
          driver_name, interpreter->allocator, &interpreter->driver),
      "failed to create driver '%.*s'", (int)driver_name.size,
      driver_name.data);
